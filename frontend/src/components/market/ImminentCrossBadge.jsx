...
function ImminentCrossBadge({ imminentCross, shortTerm = false, gatePasses = null }) {
  ...
  // (docs/quant_methodology.md §8) - un cruce bajista proyectado nunca hace
  // fallar el gate de entrada, y eso sigue siendo correcto. Pero cuando ya
  // tiene confianza suficiente para que el motor de salida actuase, un
  // lector viendo el gate en verde justo encima merece que la tensión se
  // declare explícitamente, no que quede implícita en un badge separado.
  const contradictsGate = isDeath && clearsActionBar && gatePasses === true

  return (
    <p className="entry-timing imminent-cross-badge">
      <span
        className={`sector-tier-badge sector-tier-badge--${tone}`}
        title={`Proyección, no una confirmación - no afecta al gate de entrada. ${confidenceHint}.`}
      >
        {isDeath ? '⚠️ ' : ''}
        {label}
      ...
      {contradictsGate && (
        <span className="imminent-cross-badge__conflict-note">
          · el gate de entrada aprueba por sus propias condiciones, esto es una señal del motor de salida - preguntas
          distintas, pero con la posición todavía sin abrir vale la pena esperar a que se resuelva
        </span>
      )}
    </p>
  )
