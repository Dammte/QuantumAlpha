...
import { CartesianGrid, Line, ComposedChart, ReferenceLine, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts'
...
function priceDomain(data, nearestSupport, nearestResistance) {
...
function buildChartData(data) {
  return data.map((row) => ({ ...row, ts: toTs(row.date) }))
}

function PriceChart({ data, currency, nearestSupport, nearestResistance }) {
  const chartData = useMemo(() => buildChartData(data), [data])
  const domain = useMemo(
    () => priceDomain(chartData, nearestSupport, nearestResistance),
    [chartData, nearestSupport, nearestResistance]
  )
