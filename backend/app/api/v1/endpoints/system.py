...
def _buy_dates_by_ticker(portfolio_repo: PortfolioRepository) -> dict[str, list]:
    """Parte 13: every real BUY, across every portfolio, grouped by ticker -
    what `trigger_performance_service.compute_trigger_outcomes` needs to
    tell "triggers you took" apart from "triggers you skipped". A handful of
    portfolios with a few hundred transactions each is cheap enough to just
    read in full here; this isn't a per-request hot path (Parte 12/17's
    "no live computation" rule is about the levels/triggers gate itself,
    not this one measurement endpoint)."""
    buy_dates: dict[str, list] = defaultdict(list)
    for portfolio in portfolio_repo.list_all():
        for txn in portfolio_repo.get_transactions(portfolio.id):
            if txn.transaction_type == TransactionType.BUY and txn.ticker is not None:
                buy_dates[txn.ticker].append(txn.executed_at.date())
    return dict(buy_dates)
...
def get_signal_performance(
    ...
    Parte 13 - "qué pasó con lo que no compraste": `entry_triggered` rows
    additionally split by `taken` (a real BUY of that ticker followed the
    trigger within `trigger_performance_service.TAKEN_WINDOW_DAYS`, across
    any portfolio) once enough history exists on both sides to say
    anything - comparing the two tells the propietario whether discretionary
    filtering on top of the system's own triggers is adding value or
    costing it."""
    ...
    buy_dates = _buy_dates_by_ticker(portfolio_repo)
    trigger_report = tps.build_trigger_performance_report(trigger_events, market_data, buy_dates)
